...
{
    public class EvidencijaPregledaController
    {

        private FrmKreirajEvidencijuPregleda forma;
        private BindingList<Zubar> zubari;
        private BindingList<Pacijent> pacijenti;
        private BindingList<StavkaEvidencijePregleda> stavke;
        private EvidencijaPregleda trenutnaEvidencija;
        public EvidencijaPregledaController(FrmKreirajEvidencijuPregleda forma)
        {
            this.forma = forma;
            zubari = new BindingList<Zubar>();
            pacijenti = new BindingList<Pacijent>();
            stavke = new BindingList<StavkaEvidencijePregleda>();
            trenutnaEvidencija = new EvidencijaPregleda();
            Init();
        ...
        private void Init()
        {
            try
            {
                ...
                forma.cmbPacijenti.SelectedIndex = -1;
                forma.cmbZubari.SelectedIndex = -1;
        ...
        public void DodajStavku()
        {
            ...
            trenutnaEvidencija.UkupnaCena += s.VrednostStavke;

            forma.txtUkupnaCena.Text = trenutnaEvidencija.UkupnaCena.ToString();
            forma.cmbTretman.SelectedIndex = -1;
        ...
        public void ObrisiStavku()
        {
            ...
            for (int i = selektovana.Rb; i < stavke.Count; i++)
            {
                stavke[i].Rb--;
            }

           
            trenutnaEvidencija.UkupnaCena -= selektovana.VrednostStavke;
            forma.txtUkupnaCena.Text = trenutnaEvidencija.UkupnaCena.ToString();

            stavke.Remove(selektovana);
            forma.dgvStavke.Refresh();
        }
        public void KreirajEvidenciju()
        {
            if (!ValidirajPodatke())
            {
                //MessageBox.Show("Sistem ne moze da zapamti evidenciju pregleda.", "Greska", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                Communication.Instance.PosaljiZahtevBezRezultata(Operacija.ZapamtiEvidencijuPregleda, ek);

                MessageBox.Show("Sistem je zapamtio evidenciju pregleda", "Sacuvana evidencija pregleda", MessageBoxButtons.OK, MessageBoxIcon.Information);
                OcistiFormu();
        ...
        {
            forma.datePregled.Value = DateTime.Now;
            forma.txtVremePocetka.Text = "";
            forma.txtVremeZavrsetka.Text = "";
            forma.cmbZubari.SelectedIndex = -1;
            forma.cmbPacijenti.SelectedIndex = -1;
            forma.cmbTretman.SelectedIndex = -1;
            forma.txtNapomena.Text = "";
        ...
        private bool ValidirajPodatke()
        {
            ...
            if (forma.datePregled.Value.Date < DateTime.Today)
            {
                MessageBox.Show("Datum pregleda ne može biti u prošlosti.");
                return false;
            }
